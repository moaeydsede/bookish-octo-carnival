...
export function ensureToastWrap(){
  ...
  if(!w){w=document.createElement("div");w.id="toastWrap";w.className="toastWrap noPrint";document.body.appendChild(w);}
  return w;
}
export function toast(title, body=""){
  ...
  w.appendChild(t);
  setTimeout(()=>{t.style.opacity="0.0";t.style.transform="translateY(-6px)";},3500);
...
export function openMenu(){ const b=document.getElementById("menuBack"); if(b) b.style.display="flex"; }
export function closeMenu(){ const b=document.getElementById("menuBack"); if(b) b.style.display="none"; }
export function pad(num, len){ return String(num).padStart(len,"0"); }
