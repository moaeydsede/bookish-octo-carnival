...
import { $, escapeHtml, todayISO, icon, toast, askNotifyPermission, notifyNow, openMenu, closeMenu, pad } from "./utils.js";
import { collection, doc, setDoc, getDocs, query, orderBy, limit, onSnapshot, runTransaction } from "https://www.gstatic.com/firebasejs/12.9.0/firebase-firestore.js";
...
async function sendPushToWarehouse(message){
  const url = (window.ORDERED_CONFIG && window.ORDERED_CONFIG.workerUrl) ? window.ORDERED_CONFIG.workerUrl : "";
  if(!url) return;
  try{
    await fetch(url,{
      method:"POST",
...
async function nextOrderCode(){
  ...
  const n=await runTransaction(db, async(tx)=>{
    ...
    const next = s.exists()? Number(s.data().nextNumber||1) : 1;
...
$("btnCreateOrder").onclick=async()=>{
  ...
  try{
    let orderCode=$("orderCode").value.trim();
    if(!orderCode){ orderCode = await nextOrderCode(); $("orderCode").value = orderCode; }
    const c=currentCustomer();
...
$("btnSendToWarehouse").onclick=async()=>{
  ...
  try{
    ...
    await sendPushToWarehouse(`طلبات جديدة للتجهيز: ${codes.join(", ")}`);
